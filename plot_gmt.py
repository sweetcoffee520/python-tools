...
def plot_K(xyz, spacing, region, color_scale, colorbar_type, xtitle, ytitle, unit, rows, cols) -> pygmt.Figure:
    pygmt.config(MAP_FRAME_WIDTH='2p', MAP_FRAME_TYPE='fancy',
        ...
        for i in range(cols):
            for j in range(rows):
                with fig.set_panel([j, i]):
                    ...
                    fig.colorbar(position='jCB+m+w5c/0.3c+h+o0c/-0.5c+e')
...
def plot_X(xyz, spacing, region, color_scale, colorbar_type, title, xtitle, ytitle, unit, rows, cols) -> pygmt.Figure:
    pygmt.config(MAP_FRAME_WIDTH='2p', MAP_FRAME_TYPE='fancy', MAP_TITLE_OFFSET = '2p',
        ...
        for i in range(cols):
            for j in range(rows):
                with fig.set_panel([j, i]):
                    ...
                    # backgroud表示超过色标范围的值是否用底色表示
                    cpt = pygmt.makecpt(
                        cmap=colorbar_type, series=color_scale, continuous=True, reverse=False, background=True)
                    fig.grdimage(grd, cmap=cpt, nan_transparent=True)
